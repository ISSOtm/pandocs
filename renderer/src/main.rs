...
impl Renderer for Pandocs {
    ...
    fn render(&self, ctx: &RenderContext) -> Result<()> {
        ...
        // Now, post-process the pages to inject the boxes
        let mut src = ctx.destination.join(renderer.name());
        let mut dest = ctx.destination.join(self.name());
        for (i, item) in ctx.book.iter().enumerate() {
            match item {
                BookItem::Chapter(chapter) if !chapter.is_draft_chapter() => {
                    render(&chapter, i, &mut src, &mut dest)
...
fn render(chapter: &Chapter, index: usize, src: &mut PathBuf, dest: &mut PathBuf) -> Result<()> {
    let page_name = chapter.path.as_ref().unwrap();
    src.set_file_name(page_name);
    dest.set_file_name(page_name);
    src.set_extension("html");
    ...
    let index_file = if index == 0 {
        dest.set_file_name("index.html");
        Some(File::create(&dest).context(format!("Failed to create {}", dest.display()))?)
    } else {
        None
    };
    macro_rules! output {
    ...
    for (i, line) in html.lines().enumerate() {
        ...
        if let Some(line) = line.strip_prefix("<p>:::") {
            if let Some(line) = line.strip_suffix("</p>") {
                ...
                write!(&mut stderr, "warning").unwrap();
                stderr.reset().unwrap();
                eprintln!(" ({}): ignoring \":::{}\"; box start/end tags must be alone in their paragraph", chapter.name, line);
            }
        }
        output!(line);
        output!("\n");
